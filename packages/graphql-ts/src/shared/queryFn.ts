function makeQueryFn(endpoint: string) {
  return `
   ...
   for (let [key, value] of Object.entries(resolver)) {
     if (typeof value === "object" && "arguments" in value) {
       ...
       if (typeof value.fields === "boolean") {
         queryStringArray.push(${"`${key} ${argumentsStringArray.length > 0 ? `(${argumentsStringArray.join(', ')})` : ''}`"});
       } else {
         queryStringArray.push(${"`${key} ${argumentsStringArray.length > 0 ? `(${argumentsStringArray.join(', ')})` : ''} { ${generateQueryString(value.fields).join('\\n')}}`"});
       }
     } else if (typeof value === "object") {
       queryStringArray.push(${"`${key} { ${generateQueryString(value).join('\\n')}}`"});
     } else {
